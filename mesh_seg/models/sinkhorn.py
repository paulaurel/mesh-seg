...
def solve_log_optimal_transport(assignment_matrix, num_iterations: int):
    ...
    log_mu, log_nu = log_mu[None].expand(batch_size, -1), log_nu[None].expand(batch_size, -1)

    log_assignment_matrix = perform_log_sinkhorn_iterations(assignment_matrix, log_mu, log_nu, num_iterations)
    log_assignment_matrix = log_assignment_matrix - norm
    return log_assignment_matrix
